    ...
    function attachToKo(ko) {
        ...
        ComponentInstance.prototype._getTemplateAsync = function(config, callback) {
            ...
            switch (typeof template) {
                ...
                case "function":
                    // Provider function asynchronously supplies either a node array, or a markup string.
                    // For speed, we prefer a markup string on IE<9, and a node array on all other browsers.
                    // However it should work either way on all browsers.
                    template(function(nodeArrayOrMarkupString) {
                        ...
                        if (typeof nodeArrayOrMarkupString === "string") {
                            templateSource.text(nodeArrayOrMarkupString);
                        } else {
                            // Insert into a dummy parent element. We *don't* use a DocumentFragment
                            // because IE<9 needs to be able to call .innerHTML on it as a way of cloning
                            // the nodes (and DocumentFragment doesn't have .innerHTML).
                            var anotherDummyContainer = document.createElement("div");
                            ko.virtualElements.setDomNodeChildren(anotherDummyContainer, nodeArrayOrMarkupString);
                            templateSource.nodes(anotherDummyContainer);
                        }
                        callback(dummyContainer);
                    });
                    break;
        ...
        AjaxLoader.prototype.load = function(url) {
            var self = this;
            if (!self._promises[url]) {
                // TODO: Drop the jQuery dependency
                // Also consider ensuring that pre-cached results are returned synchronously
                // (not sure whether jQuery Ajax promises fire 'done' synchronously if already completed)
                self._promises[url] = $.get(url)
                    .fail(function(req, result, message) {
                        // Retry loading next time
                        delete self._promises[url];
                        throw new Error("Error loading " + url + ": " + message);
                    });
            }

            return self._promises[url];
        }

        var originalDocumentCreateDocumentFragment,
            componentConfigRegistry = {},
            templateLoader = new AjaxLoader();
        ko.components = {
            register: function(name, options) {
                ...
                // Support loading templates via Ajax
                if (options.templateUrl && !options.template) {
                    options.template = function(callback) {
                        templateLoader.load(options.templateUrl).done(function(markup) {
                            var nodeArray = ko.utils.parseHtmlFragment(markup);
                            callback(nodeArray);
                        });
                    };
                }
